...
<template>
    <ul :class="containerClass" v-if="!noLiSurround">
        <li v-if="firstLastButton" :class="[pageClass, firstPageSelected() || isLoadList == false ? disabledClass : '']">
        ...
        <li v-if="!(firstPageSelected() && hidePrevNext)"
            :class="[prevClass, firstPageSelected() || isLoadList == false ? disabledClass : '']">
            <a @click="prevPage()" @keyup.enter="prevPage()" :class="prevLinkClass" :tabindex="firstPageSelected() ? -1 : 0"
                v-html="prevText"></a>
        </li>

        <li v-for="page in pages" :key="page.index" :class="[
            pageClass,
            page.selected && this.currentPage != 1 ? activeClass : '',
            page.disabled ? disabledClass : '',
            page.breakView ? breakViewClass : '',
            page.content == this.currentPage ? activeClass : '',
            isLoadList == false ? disabledClass : ''
        ]">
        ...
        <li v-if="!(lastPageSelected() && hidePrevNext)"
            :class="[nextClass, lastPageSelected() || isLoadList == false ? disabledClass : '']">
            <a @click="nextPage()" @keyup.enter="nextPage()" :class="nextLinkClass" :tabindex="lastPageSelected() ? -1 : 0"
                v-html="nextText"></a>
        </li>

        <li v-if="firstLastButton" :class="[pageClass, lastPageSelected() || isLoadList == false ? disabledClass : '']">
    ...
    <div :class="containerClass" v-else>
        ...
        <template v-for="page in pages">
            ...
            <a v-else-if="page.disabled" :key="page.index" :class="[
                pageLinkClass,
                page.selected ? activeClass : '',
                disabledClass,
            ]" tabindex="0">{{ page.content }}</a>
            <a v-else :key="page.index" @click="handlePageSelected(page.index + 1)"
